...
All the third-party (open source) dependencies are linked to a specific version to keep build reproducible and to prevent supply chain attacks (cryptographic checksums are used instead or on top of version numbers are used wherever possible to pin the version).
